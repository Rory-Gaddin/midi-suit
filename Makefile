...
AVRDUDE_PORT = /dev/tty.usbserial-A600eArn #A700em75    programmer connected to serial device
...
# Default target.
all: begin gccversion sizebefore clean build 
...
upload: $(OBJDIR)/$(TARGET).hex $(OBJDIR)/$(TARGET).eep
	$(AVRDUDE) -b $(AVRDUDE_SPEED) $(AVRDUDE_FLAGS) $(AVRDUDE_WRITE_FLASH) $(AVRDUDE_WRITE_EEPROM)
